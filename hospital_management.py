            ...
            for patient in root.findall('Patients/patient'):
                ...
                if fname_element is not None and fname_element.text is not None:
                    ...
                    if fname.lower() == search_name.lower():
                        ...
                        full_name = "{} {} {}".format(
                            patient.find('Name/fname').text,
                            patient.find('Name/mname').text,
                            patient.find('Name/lname').text
                        )
                        st.write("Patient Name:", full_name)

                        # Display patient information
                        st.write("SSN:", patient.find('SSN').text)
                        st.write("Age:", patient.find('age').text)
                        st.write("Room No:", patient.find('roomno').text)
                        st.write("Medical Problem:", patient.find('Medicalproblem').text)
                        st.write("Drug Allergy:", patient.find('Drugallergy').text)
                        st.write("Primary Insurance ID:", patient.find('Insurance[@type="Primary"]/P.id').text)
                        st.write("Primary Insurance Address:", patient.find('Insurance[@type="Primary"]/P.address').text)
                        st.write("Secondary Insurance ID:", patient.find('Insurance[@type="Secondary"]/S.id').text)
                        st.write("Secondary Insurance Address:", patient.find('Insurance[@type="Secondary"]/S.address').text)
...
def delete_patient():
    ...
    # Create a list of patient names for autofill suggestions
    patient_names = [patient.find('Name/fname').text.capitalize() for patient in root.findall('Patients/patient')]

    # Create selectbox for patient names with autofill
    delete_name = st.selectbox("Select Patient Name to Delete", options=patient_names, key="delete_name_select")

    # Find the patient element based on the selected name
    selected_patient = None
    for patient in root.findall('Patients/patient'):
        fname_element = patient.find('Name/fname')
        if fname_element is not None and fname_element.text is not None:
            fname = fname_element.text.capitalize()
            if fname == delete_name:
    ...
    if selected_patient:
        ...
        st.write("SSN:", selected_patient.find('SSN').text)
        st.write("Age:", selected_patient.find('age').text)
        st.write("Room No:", selected_patient.find('roomno').text)
        st.write("Medical Problem:", selected_patient.find('Medicalproblem').text)
        st.write("Drug Allergy:", selected_patient.find('Drugallergy').text)
        st.write("Primary Insurance ID:", selected_patient.find('Insurance[@type="Primary"]/P.id').text)
        st.write("Primary Insurance Address:", selected_patient.find('Insurance[@type="Primary"]/P.address').text)
        st.write("Secondary Insurance ID:", selected_patient.find('Insurance[@type="Secondary"]/S.id').text)
        st.write("Secondary Insurance Address:", selected_patient.find('Insurance[@type="Secondary"]/S.address').text)

        # Create a button to confirm deletion
        if st.button("Confirm Deletion"):
            # Remove the selected patient element from the root
            root.find('Patients').remove(selected_patient)
        ...
        st.warning("No patient found with the selected name.")
